...
def get_client_ip():
    # Helper to consistently grab the spoofed IP from the curl header
...
def log_successful_request(response):
    if response.status_code != 429:
        log_request(get_client_ip(), response.status_code)
    return response
